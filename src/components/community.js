...
document.addEventListener("DOMContentLoaded", function () {
  ...
  likeButtons.forEach((button, index) => {
    ...
    button.addEventListener('click', function () {
      // 해당 버튼에 연결된 이미지 요소를 가져옴
      const imgElement = document.getElementById(`like-img-${index + 1}`);
      // 해당 버튼에 연결된 좋아요 수를 표시하는 요소를 가져옴
      const countElement = document.getElementById(`like-count-${index + 1}`);
